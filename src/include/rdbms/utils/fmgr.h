...
typedef struct FmgrInfo {
  ...
  bool fn_ret_set;        // Function returns a set
...
typedef struct FunctionCallInfoData {
  ...
  bool arg_null[FUNC_MAX_ARGS];  // T if arg[i] is actually NULL
} FunctionCallInfoData;

// This routine fills a FmgrInfo struct, given the OID
// of the function to be called.
void fmgr_info(Oid function_id, FmgrInfo* finfo);
...
#define PG_ARG_IS_NULL(n) (fcinfo->arg_null[n])
...
#define PG_DETOAST_DATUM(datum) \
  pg_detoast_datum((struct VarLena*)DATUM_GET_POINTER(datum))
#define PG_DETOAST_DATUM_COPY(datum) \
  pg_detoast_datum_copy((struct VarLena*)DATUM_GET_POINTER(datum))
...
#define PG_GETARG_VARCHAR_P_COPY(n) \
  DATUM_GET_VAR_CHAR_P_COPY(PG_GETARG_DATUM(n))
...
typedef struct {
  int api_version;  // pecifies call convention version number
  // More fields may be added later, for version numbers > 1.
} PgFInfoRecord;

typedef PgFInfoRecord* (*PGFInfoFunction)(void);

// Macro to build an info function associated with the given function name.
#define PG_FUNCTION_INFO_V1(funcname)                          \
  extern PgFInfoRecord* cpp_concat(pg_finfo_, funcname)(void); \
  PgFInfoRecord* cpp_concat(pg_finfo_, funcname)(void) {       \
    static PgFInfoRecord my_finfo = {1};                       \
    return &my_finfo;                                          \
  }
