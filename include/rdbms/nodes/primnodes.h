...
#ifndef RDBMS_NODES_PRIM_NODES_H_
#define RDBMS_NODES_PRIM_NODES_H_

#include "rdbms/access/attnum.h"
#include "rdbms/nodes/pg_list.h"
#include "rdbms/utils/fcache.h"
...
typedef enum OpType { OP_EXPR, FUNC_EXPR, OR_EXPR, AND_EXPR, NOT_EXPR, SUBPLAN_EXPR } OpType;

typedef struct Expr {
  NodeTag type;
  Oid type_oid;    // Oid of the type of this expr.
  OpType op_type;  // Type of the op.
...
typedef struct Var {
  NodeTag type;
  Index varno;
  AttrNumber var_attno;
  Oid var_type;
  int32 var_type_mod;
  Index var_levels_up;  // Erased by upper optimizer.
  Index varno_old;      // Mainly for debugging --- see above.
...
//      constisnull - whether the constant is null (if true, the other fields are undefined)
...
typedef struct Iter {
  NodeTag type;
  Node* iter_expr;
  Oid iter_type;  // Type of the iter expr (use for type checking).
} Iter;
...
typedef enum SubLinkType { EXISTS_SUBLINK, ALL_SUBLINK, ANY_SUBLINK, MULTIEXPR_SUBLINK, EXPR_SUBLINK } SubLinkType;

typedef struct SubLink {
  NodeTag type;
  SubLinkType sublink_type;
  bool use_or;
  List* lefthand;
  List* oper;
  Node* subselect;
...
typedef struct Array {
  NodeTag type;
  Oid array_elem_type;
  int array_elem_length;
  bool array_elem_by_val;
  int array_ndim;
  IntArray array_low;
  IntArray array_high;
  int array_len;
...
typedef struct ArrayRef {
  ...
  int ref_elem_length;
  Oid ref_elem_type;
  bool ref_elem_by_val;
  List* ref_upper_ind_expr;
  List* ref_lower_ind_expr;
  Node* ref_expr;
  Node* ref_assgn_expr;
} ArrayRef;
...
typedef struct RelabelType {
  ...
  Node* arg;
  Oid result_type;
  int32 result_type_mod;
} RelabelType;

#endif  // RDBMS_NODES_PRIM_NODES_H_
