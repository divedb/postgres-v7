...
#if defined(SUN_LEN)
#define UNIXSOCK_PATH(sun, port) (sprintf((sun).sun_path, "/tmp/.s.PGSQL.%d", (port)), SUN_LEN(&(sun)))
#else
#define UNIXSOCK_PATH(sun, port) \
  (sprintf((sun).sun_path, "/tmp/.s.PGSQL.%d", (port)), strlen((sun).sun_path) + offsetof(struct sockaddr_un, sun_path))
#endif
