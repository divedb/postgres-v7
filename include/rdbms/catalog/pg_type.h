...
//
//===----------------------------------------------------------------------===//
#ifndef RDBMS_CATEGORY_PG_TYPE_H_
#define RDBMS_CATEGORY_PG_TYPE_H_
...
DATA(insert OID = 22(int2vector PGUID INDEX_MAX_KEYS  // 2 -
                     1 f b t \054 0 21 int2vectorin int2vectorout int2vectorin int2vectorout i p _null_));
...
DATA(insert OID = 30(oidvector PGUID INDEX_MAX_KEYS  // 4 -
                     1 f b t \054 0 26 oidvectorin oidvectorout oidvectorin oidvectorout i p _null_));
...
DATA(insert OID =
         704(tinterval PGUID 12 47 f b t \054 0 0 tintervalin tintervalout tintervalin tintervalout i p _null_));
...
DATA(insert OID =
         1184(timestamp PGUID 8 47 f b t \054 0 0 timestamp_in timestamp_out timestamp_in timestamp_out d p _null_));
DESCR("date and time");
...
DATA(insert OID =
         1186(interval PGUID 12 47 f b t \054 0 0 interval_in interval_out interval_in interval_out d p _null_));
...
extern Oid type_get(char* type_name, bool* defined);
extern Oid type_shell_make(char* type_name);
extern Oid type_create(char* type_name, Oid assigned_type_oid, Oid relation_oid, int16 internal_size,
                       int16 external_size, char type_type, char type_delim, char* input_procedure,
                       char* output_procedure, char* receive_procedure, char* send_procedure, char* element_type_name,
                       char* default_type_value, bool passed_by_value, char alignment, char storage);
extern void type_rename(const char* old_type_name, const char* new_type_name);
extern char* make_array_type_name(char* type_name);

#endif  // RDBMS_CATEGORY_PG_TYPE_H_
