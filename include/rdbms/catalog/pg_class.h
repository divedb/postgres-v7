...
#ifndef RDBMS_CATALOG_PG_CLASS_H_
#define RDBMS_CATALOG_PG_CLASS_H_
...
CATALOG(pg_class) BOOTSTRAP {
  ...
  Oid relam;         // 访问方法的Oid
  int4 relpages;     // 表的页数量
  int4 reltuples;    // 表的tuple数量
  Oid rellongrelid;
  bool relhasindex;  // 是否有索引
  bool relisshared;  // 是否共享
  char relkind;
...
#define CLASS_TUPLE_SIZE (offsetof(FormData_pg_class, relhasrules) + sizeof(bool))

// Form_pg_class corresponds to a pointer to a tuple with the format of pg_class relation.
